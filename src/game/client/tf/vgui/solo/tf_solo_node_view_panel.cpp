...
void ConfirmSelectSoloNode(bool bConfirmed, void* pContext)
{
	if (bConfirmed)
	{
		CSoloNodeViewSubPanel* pQuestViewSubpanel = (CSoloNodeViewSubPanel*)(pContext);
...
// Purpose:
//-----------------------------------------------------------------------------
void CSoloObjectivePanel::PerformLayout()
{
...
// Purpose:
//-----------------------------------------------------------------------------
CSoloObjectiveTooltip::CSoloObjectiveTooltip(vgui::Panel* parent, const char* text)
...
// Purpose:
//-----------------------------------------------------------------------------
void CSoloObjectiveTooltip::PerformLayout()
{
	BaseClass::PerformLayout();
	...
	CSoloNodeViewSubPanel* pItemPanel = m_hCurrentPanel.Get();
...
// Purpose:
//-----------------------------------------------------------------------------
void CSoloObjectiveTooltip::ShowTooltip(Panel* currentPanel)
{
	if (m_pObjectivePanel && currentPanel != m_hCurrentPanel.Get())
	{
		CSoloNodeViewSubPanel* pItemPanel = assert_cast<CSoloNodeViewSubPanel*>(currentPanel);
...
// Purpose:
//-----------------------------------------------------------------------------
void CSoloObjectiveTooltip::HideTooltip()
{
...
CSoloNodeViewSubPanel::CSoloNodeViewSubPanel(Panel* pParent,
	const char* pszPanelName)
	: CExpandablePanel(pParent, pszPanelName)
	, m_pToolTip(NULL)
{
	m_pObjectivePanel = new CSoloObjectivePanel(this, "objectives");
...
void CSoloNodeViewSubPanel::PerformLayout()
{
	...
	bool bCanChoose = true;//pNodeDef->BCanUnlock(GetQuestMapHelper());

	const bool bActive = false;// m_pQuest&& m_pQuest->Obj().active();
	const bool bCompleted = false; // m_pQuest&& m_pQuest->BEarnedAllPointsForCategory(QUEST_POINTS_NOVICE) &&
	...
	const bool bCanTurnIn = false; //GetQuestMapHelper().BCanNodeBeTurnedIn(m_msgNodeData.defindex());
...
void CSoloNodeViewSubPanel::OnSizeChanged(int nWide, int nTall)
{
...
void CSoloNodeViewSubPanel::OnCommand(const char* command)
{
...
void CSoloNodeViewSubPanel::BeginTurnInAnimation()
{
...
//void CSoloNodeViewSubPanel::SetQuestData(const CQuest* pQuest, const CQuestDefinition* pQuestDef, bool bShowObjectives)
//{
...
void CSoloNodeViewSubPanel::FireGameEvent(IGameEvent* event)
{
...
//void CSoloNodeViewSubPanel::SetNodeData(const CSOQuestMapNode& msgNodeData)
//{
//	m_msgNodeData = msgNodeData;
//	InvalidateLayout();
//}

void CSoloNodeViewSubPanel::OnConfirmSelectQuest()
{
	//GetQuestMapController().SelectNodeQuest(m_msgNodeData, m_pQuestDef->GetDefIndex());
}

void CSoloNodeViewSubPanel::OnShowTurnInSuccess()
{

}

void CSoloNodeViewSubPanel::OnCursorEntered()
{
	//if (!m_pQuest)
	//{
...
void CSoloNodeViewSubPanel::OnCursorExited()
{
	//if (!m_pQuest)
	//{
	//	HideSelectQuestInfo();
...
void CSoloNodeViewSubPanel::HideSelectQuestInfo()
{
	g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(this, "QuestViewSubPanel_SelectMouseExit");
}

CSoloNodeViewPanel::CSoloNodeViewPanel(Panel* pParent, const char* pszPanelname)
	: BaseClass(pParent, pszPanelname)
	, m_pItemToolTip(NULL)
	, m_pToolTip(NULL)
	, m_bQuestCreated(false)
{
	...
	{
		CSoloNodeViewSubPanel* pQuestTrackerPanel = new CSoloNodeViewSubPanel(m_pContentContainer
...
void CSoloNodeViewPanel::UpdateQuestSubPanels()
{
	/*
...
void CSoloNodeViewPanel::PerformLayout()
{
	/*
	if (m_bTurningIn)
		return;
...
void CSoloNodeViewPanel::UpdateHeights()
{
	//
	// Expanded
	//
...
void CSoloNodeViewPanel::SelectedQuestArrived()
{
	m_bQuestCreated = false;
	UpdateQuestSubPanels();
	InvalidateLayout();
...
void CSoloNodeViewPanel::OnCommand(const char* pCommand)
{
...
void CSoloNodeViewPanel::OnThink()
{
	BaseClass::OnThink();
...
void CSoloNodeViewPanel::FireGameEvent(IGameEvent* event)
{
	if (FStrEq("quest_response", event->GetName()))
	{
		int nRequest = event->GetInt("request");
...
void CSoloNodeViewPanel::SetTextTooltip(CTFTextToolTip* pTooltip)
{
	m_pToolTip = pTooltip;
...
void CSoloNodeViewPanel::SetObjectiveTooltip(CSoloObjectiveTooltip* pObjectiveTooltip)
{
	FOR_EACH_VEC(m_vecQuestSubPanels, i)
	{
...
/*
void CSoloNodeViewPanel::SetData(const CSOQuestMapNode& msgNodeData)
{
...
void CSoloNodeViewPanel::UpdateQuestViewPanelForNode(CSoloNodePanel* pNodePanel)
{
...
void CSoloNodeViewPanel::SetVisible(bool bVisible)
{
...
void CSoloNodeViewPanel::UpdatePosition()
{
...
void CSoloNodeViewPanel::TurnInComplete()
{
	m_bTurningIn = false;
	InvalidateLayout(true);
